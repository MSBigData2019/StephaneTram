...
def getSoupFromUrl(url,stock_name):
    res = requests.get(url+stock_name+'.html')
...
def getModulesFromColumns(soup):
    ...
    columns=soup.find_all('div',{'class':'prix_barre_liste'})
    modules = [item for sublist in [x.find_all('p', class_='darty_prix_barre_remise darty_small separator_top') for x in columns] for item in sublist]
    print(modules)
    return modules
...
def launch_requests(stock_name):
    ...
    dataDict = parseModules(getModulesFromColumns(soup))
    print(dataDict)
    return dataDict
