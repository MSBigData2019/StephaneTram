...
def occurences(text):
  return {k:text.lower().count(k) for k in 'abcdefghijklmnopqrstuvwxyz'}
...
def pigLatin(text):
  return ''.join([x[1:] + x[0] + 'ay' + ' ' for x in text.split(" ")]).lower().capitalize()[:-1]
...
def flatten():
    return [(x['nombre_de_reservation'],x['auteur'],x['timestamp']) for x in response['records']]
